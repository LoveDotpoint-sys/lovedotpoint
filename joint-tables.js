...
(function(){
  ...
  function jointNos(id){
    ...
    const s=sessionRow(id),saved=Array.isArray(s?.joined_table_nos)?s.joined_table_nos.map(Number).sort((a,b)=>a-b):[];
  ...
  function repaintFloor(){
    document.querySelectorAll('.tablecard[data-table-no]').forEach(card=>{
      const no=Number(card.dataset.tableNo),t=tables.find(x=>Number(x.table_no)===no);if(!t?.active_session_id)return;
  ...
  function injectOrderJointBar(){
    ...
    bar.innerHTML=`<div><span>${nums.length>1?'JOINT TABLE ACTIVE':'TABLE SERVICE'}</span><b>${nums.length>1?'Tables '+escJoint(label):'Table '+escJoint(label)}</b><small>${nums.length>1?'One combined order · one bill · all joined tables release together':'Need extra seating? Join available tables into this same order.'}</small></div><button class="btn ghost" onclick="manageJointTables('${selectedSession}',${Number(selectedTable)})">${nums.length>1?'Manage Joint Tables':'＋ Join Tables'}</button>`;
  ...
  window.unjoinOneTable=async function(sessionId,tableNo){
    const nums=jointNos(sessionId);if(nums.length<=1)return toast('Last table cannot be unjoined');
    if(!confirm(`Unjoin Table ${tableNo}?\n\nCurrent order will remain on the other joined tables.`))return;
    const {data,error}=await sb.rpc('unjoin_table',{p_session_id:sessionId,p_table_no:tableNo});if(error)return toast(error.message);
    await refreshAll();
    if(String(selectedSession||'')===String(sessionId)&&Number(selectedTable)===Number(tableNo))selectedTable=(data||[])[0]||null;
    manageJointTables(sessionId,selectedTable||((data||[])[0]));toast(`Table ${tableNo} released from joint group`);
  };
  ...
  if(typeof baseShared==='function')window.viewSharedTable=async function(no){
    await baseShared(no);const t=tables.find(x=>Number(x.table_no)===Number(no));if(!t?.active_session_id)return;
    ...
    if(me?.role==='owner'){
      ...
      if(actions&&!document.getElementById('manageJointBtn'))actions.insertAdjacentHTML('afterbegin',`<button id="manageJointBtn" class="btn ghost" onclick="manageJointTables('${t.active_session_id}',${no})">${nums.length>1?'Manage Joint Tables':'＋ Join Tables'}</button>`);
    }
  };
